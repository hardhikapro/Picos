...
import com.example.picos.databinding.FragmentPregnantBinding
import com.example.picos.ui.viewModel.SelfAssessmentViewModel


class PregnantFragment : Fragment(), View.OnClickListener {
    private var selectedOption: String = ""
    private var _binding: FragmentPregnantBinding? = null
    private val binding get() = _binding!!
    private lateinit var assessmentViewModel: SelfAssessmentViewModel
    private lateinit var pregnant:String
    ...
    override fun onCreateView(
        ...
        _binding = FragmentPregnantBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val btnNext: Button = binding.btnPregnantNext
        btnNext.setOnClickListener(this)
        assessmentViewModel = ViewModelProvider(requireActivity())[SelfAssessmentViewModel::class.java]
        val optOne: TextView = binding.tvOptOnePregnant
        val optTwo: TextView = binding.tvOptTwoPregnant
    ...
    override fun onClick(v: View?) {
        when (v?.id) {
            R.id.tv_optOnePregnant -> {
                selectedOptColor(binding.tvOptOnePregnant, 1)
                pregnant = "1"
            }

            R.id.tv_optTwoPregnant -> {
                selectedOptColor(binding.tvOptTwoPregnant, 2)
                pregnant = "0"
            }

            R.id.btnPregnant_next -> {
                assessmentViewModel.Pregnant = pregnant
                val fragment = WeightGainFragment()
    ...
    private fun setMensAppearance() {
        ...
        options.add(binding.tvOptOnePregnant)
        options.add(binding.tvOptTwoPregnant)
